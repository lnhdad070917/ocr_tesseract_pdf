...
def pdfminer_to_text(input_file:str, page_no:int=None):
    ...
    output_file = f'./results/pdfminer_{input_file}.txt' if page_no is None else f'./results/pdfminer_{input_file}_page{page_no}.txt'
...
def tika_to_text(input_file):
    parsed_pdf = parser.from_file("./data/" + input_file + ".pdf")
    text = parsed_pdf["content"]
    output_file = f'./results/tika_{input_file}.txt'
    with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
        f.write(text)
    return text
...
def pdfminer_to_xml(input_file:str, page_no:int=None):
    ...
    for page_index, page in enumerate(PDFPage.get_pages(fp)):
        if page_no is None or page_index == page_no:
            ...
            outfp.truncate(0)
            if page_no and page_index == page_no: break
    output_file = f'./results/pdfminer_{input_file}.xml' if page_no is None else f'./results/pdfminer_{input_file}_page{page_no}.xml'
    with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
        f.write(text)
    return text
...
def pdfminer_to_html(input_file:str, page_no:int=None):
    ...
    for page_index, page in enumerate(PDFPage.get_pages(fp)):
        if page_no is None or page_index == page_no:
            ...
            text += retstr
            output_file = f'./results/pdfminer_{input_file}_page{page_index}.html'
            with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
                f.write(retstr)
            if page_no and page_index == page_no: break
            outfp.seek(0)
...
def pdfminer_to_html_char_level(input_file:str, page_no:int=None):
    ...
    for page_index, page in enumerate(pages):
        output_file = f'./results/pdfminer_char_{input_file}_page{page_index}.html'
        with open(output_file, 'w', encoding='utf-8', errors='ignore') as f:
            f.write(body_start)
